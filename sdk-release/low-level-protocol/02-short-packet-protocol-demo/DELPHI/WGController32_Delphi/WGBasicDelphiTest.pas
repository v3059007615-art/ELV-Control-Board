...
begin
    ...
    Memo1.Lines.Add(format('%02u:%02u:%02u %s',[Hour, Min, Sec, info]));
...
begin
    ...
    displayRecordInformation(recvBuff);
		...
		if ((otherInputStatus and 1) > 0) then log('Force locking');
		if ((otherInputStatus and 2) > 0)  then  log('Fire!');
   ...
   //Control Current Time
    logStr(format('  controller time: 20%02X-%02X-%02X %02X:%02X:%02X',[recvBuff[51],recvBuff[52],recvBuff[53],recvBuff[37],recvBuff[38],recvBuff[39]]));
  end
	else
	begin
    ...
    ret :=  pktrun(sendBuff, recvBuff);
     ...
     logStr(format('  controller time: %02X%02X-%02X-%02X %02X:%02X:%02X',[recvBuff[8],recvBuff[9],recvBuff[10],recvBuff[11],recvBuff[12],recvBuff[13],recvBuff[14]]));
    end;
     ...
     ret :=  pktrun(sendBuff, recvBuff);
      ...
      success :=1;
      	//12	Record type
				...
				//0xFF=The record indicating the given index position has been overwritten.  Use the index.0, Retrieving index values from the earliest record
				recordType := recvBuff[12];
				if (recordType = 0) then break; //No more records.
  ...
  i := WGPacketShort.SpecialFlag;
   ...
   cardNOOfPrivilege := $0037D70D;
    ...
    sendBuff[1] :=  $54;
    CopyMemory(@(sendBuff[4]),@controllerSN,4);
   	//	Identification(Prevent Error Settings)	1	0x55 [Fixed]
    i := WGPacketShort.SpecialFlag;
   ...
   cardNOOfPrivilege := $0037D70D;
    ...
    Begin
		success :=1;
    		if (CompareMem(@cardNOOfPrivilege,@(recvBuff[8]),4)) then log('1.15     Can not open message...')
        else log('1.15      When no permission: (The card number is0)');
    ...
    cardNOOfPrivilege := $1; // 'Index number(From1Start)
    CopyMemory(@(sendBuff[8]),@cardNOOfPrivilege,4);
    ret :=  pktrun(sendBuff, recvBuff);
  	success :=0;
  	if (ret = 1)  then
    ...
    CopyMemory(@cardNOOfPrivilege,@(recvBuff[8]),4);
        if ( cardNOOfPrivilege = $ffffffff)  then   log('1.16      Can not open message: (Permissions deleted)')
        else if  ( cardNOOfPrivilege = $0)  then  log('1.16       Can not open message: (The card number is0)--This index number is no longer valid.')
        else log('1.16      Can not open message...');
    ...
    CopyMemory(@(sendBuff[4]),@controllerSN,4);
	  ...
	  sendBuff[8 +0] := $02; //2Door.
	  sendBuff[8 +1] := $03; //Online
	  sendBuff[8 +2] := $03; //Open the door late.
    ret :=  pktrun(sendBuff, recvBuff);
  	success :=0;
  	if (ret = 1)  then
    ...
    cardCount := 10000;  //2015-06-09 20:20:20 Total number of cards
    ...
    begin
               ...
               CopyMemory(@(sendBuff[8]),@cardNOOfPrivilege,4);
                ...
                j:=i+1;
                CopyMemory(@(sendBuff[35]),@j,4);   //The index place for the current permission(From1Start)
  ...
  if (ret = 1) then log('Basic function test Success...')
     else  log('Basic function test Failed????...');
end;
