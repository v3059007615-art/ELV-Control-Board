...
public:
	...
	int runWithPassword(ACE_SOCK_CODgram udp,unsigned char* password)  //2015-11-02 15:39:39 Communication password ByUDPSend Command Can not open message
	{
		unsigned char buff[WGPacketSize];
		int errcnt =0;
		WGPacketShort::sequenceId++;
		...
		unsigned int currentSequenceId = WGPacketShort::sequenceId;
		memcpy(&(buff[40]),&(currentSequenceId), 4);
		ShortEncrypt((char*)buff,(char*)password);  //2015-11-02 15:40:59 Encryption
		int tries =3;
		do 
		{
			if (-1 == udp.send (buff, WGPacketSize))
			...
			else 
			{
				...
				if (recv_cnt == WGPacketSize)
				{
					ShortDecrypt((char*)recv,(char*)password);  //2015-11-02 15:40:59 Decrypt
	...
	{
		return runWithPassword1024(udp,buff1024, NULL) ;
	}

	int runWithPassword1024(ACE_SOCK_CODgram udp,unsigned char* buff1024,unsigned char* password)  //1024Bytes 2015-11-02 15:39:39 Communication password ByUDPSend Command Can not open message
	{
		...
		if (password != NULL)  
		{
			...
			{
				memcpy(&(buff[0]),&(buff1024[i]),WGPacketSize);
				ShortEncrypt((char*)buff,(char*)password);  //2015-11-02 15:40:59 Encryption
				memcpy(&(buff1024[i]),&(buff[0]),WGPacketSize);
			}
		}
		int tries =3;
		do 
			...
			else 
			{
				...
				if (recv_cnt == 1024)
				{
					if (password != NULL)  
					{
						for (int i = 0; i < 1024; i = i + 64)
						{
							memcpy(&(recv[0]),&(recv1024[i]),WGPacketSize);
							ShortDecrypt((char*)recv,(char*)password);  //2015-11-02 15:40:59 Decrypt
							memcpy(&(recv1024[i]),&(recv[0]),WGPacketSize);
...
int testBasicFunction(char *ControllerIP, unsigned int controllerSN)  //Basic function test
{
	...
	if (success == 0)
	{
		ret = pkt.runWithPassword(udp,commPassword);  //2015-11-02 10:21:22 Try the password operation for the controller.
	...
	ret = pkt.runWithPassword(udp,commPassword);  //
	success =0;
	if (ret > 0)
	{
		if (pkt.recv[8] == 1)
	...
	long validRecordsCount = 0;
        ...
        if (ret > 0)
        {
            ...
            do
            {
                ...
                ret = pkt.run1024(udp,command1024);
                success = 0;
                if (ret > 0)
                {
                    for (int j = 0; j < 1024; j = j + 64)
		...
		//1.21	Permissions added from childhood to larger[Function Number: 0x56] Applies to privileges1000, Less810,000 **********************************************************************************
            ...
            for (int i = 0; i < cardCount; )
            {
                ...
                ret = pkt.run1024(udp,command1024); //2015-11-04 19:15:48 
                success = 0;
                if (ret > 0)
                {
                    if (pkt.recv[8] == 1)
