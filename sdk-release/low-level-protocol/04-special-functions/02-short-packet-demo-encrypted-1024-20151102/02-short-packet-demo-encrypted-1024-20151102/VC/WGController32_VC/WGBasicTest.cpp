...
public:
	...
	int runWithPassword(ACE_SOCK_CODgram udp,unsigned char* password)  //2015-11-02 15:39:39 通信密码 通过指定的UDP发送指令 接收返回信息
	{
		unsigned char buff[WGPacketSize];
		int errcnt =0;
		WGPacketShort::sequenceId++;
		...
		unsigned int currentSequenceId = WGPacketShort::sequenceId;
		memcpy(&(buff[40]),&(currentSequenceId), 4);
		ShortEncrypt((char*)buff,(char*)password);  //2015-11-02 15:40:59 加密
		int tries =3;
		do 
		{
			if (-1 == udp.send (buff, WGPacketSize))
			...
			else 
			{
				...
				if (recv_cnt == WGPacketSize)
				{
					ShortDecrypt((char*)recv,(char*)password);  //2015-11-02 15:40:59 解密
	...
	int runWithPassword1024(ACE_SOCK_CODgram udp,unsigned char* buff1024,unsigned char* password)  //1024字节 2015-11-02 15:39:39 通信密码 通过指定的UDP发送指令 接收返回信息
	{
...
int testBasicFunction(char *ControllerIP, unsigned int controllerSN)  //基本功能测试
{
	...
	if (success == 0)
	{
		ret = pkt.runWithPassword(udp,commPassword);  //2015-11-02 10:21:22 再尝试控制器已有密码的操作
